...
**Используемые данные:**
- Фотографии фасадов зданий (источник: сайт citywalls.ru, панорамы Яндекс карт)  
- Исторические справки (источник: краеведческие базы)

> Примеры данных доступны в репозитории в папке `/data/samples/` (включая изображения и специализированную разметку, пригодную для обучения модели OS2D).
...
- **Результаты бейзлайна**:  
  - mAP@0.5 = 72%  
  - Время инференса на RTX 3050: 5 мс

#### Стратегия развития решения
1. **Замена бейзлайна на OS2D** — специализированная архитектура для one-shot detection  
2. **Fine-tuning на пользовательских данных**  
3. **Оптимизация**:  
   - Квантование FP16 → INT8 (опционально, если не будет наблюдаться заметная потеря точности)  
   - ONNX → TensorRT конвертация для мобильных устройств

#### Анализ и интерпретация
- Использование **Grad-CAM** для визуализации внимания модели на архитектурные элементы  
- Построение **t-SNE** embedding space для оценки разделения классов  
- Проверка согласованности с текстовыми описаниями через **ruCLIP Score**

#### Риски и меры противодействия
| Риск | Мера противодействия |
|------|----------------------|
| Низкая точность на редких объектах | Аугментация + few-shot synthesis |
| Долгое время инференса на слабых устройствах | Квантование, TensorRT, адаптивное разрешение |
| Нестабильность one-shot классификации | Использование ансамбля embedding-моделей |
| Переобучение из-за малого числа классов | Regularization (DropPath, label smoothing) |

#### Необходимый результат этапа
- Обученная модель в формате .pt, .onnx, ,tensort (in process)
...
Система состоит из двух логически независимых частей:  
- **Мобильное приложение** — выполняет локальный инференс (детекцию и классификацию) без интернета  
- **Backend-сервис** — предоставляет историческую справку по ID объекта через REST API

```mermaid
graph LR
    A[Мобильное приложение] -->|1. Обработка фото на устройстве| B[Локальная ML-модель]
    B -->|2. ID объекта| C[HTTP-запрос к API]
    C --> D[API Gateway]
    D --> E[Сервис исторических справок]
    E --> F[(База данных: PostgreSQL)]
    F -->|3. JSON-справка| E --> D --> A
```
...
|---------|-----------|-----------|
| Мобильное приложение | Съёмка фото, UI, локальный инференс | React Native + TensorFlow Lite (Android) / Core ML (iOS) |
| Локальная ML-модель | One-shot детекция и классификация зданий | OS2D, оптимизированная до 87 МБ, формат TensorRT / Core ML |
| API Gateway | Приём запросов, аутентификация, rate-limiting | AWS API Gateway |
| Сервис исторических справок | Возврат текстового описания по ID | Python FastAPI |
| База данных | Хранение структурированных исторических справок | PostgreSQL (AWS RDS) |

> **Приватность**: Фотографии **никогда не покидают устройство**. На сервер передаётся **только числовой ID объекта**.

### 4.2. Инфраструктура

Система развернута в **Amazon Web Services (AWS)**:

- **Вычисления**: 2× AWS EC2 `t3.medium` в разных Availability Zones  
- **База данных**: AWS RDS PostgreSQL (`db.t4g.small`, Multi-AZ, ежедневные бэкапы)  
- **API**: AWS API Gateway + Application Load Balancer  
- **Мониторинг**: CloudWatch (алерты при >80% CPU или ошибках 5xx)  
- **Хранение (резерв)**: Amazon S3 (для будущих логов и метаданных)

Архитектура обеспечивает **высокую доступность** за счёт:
- Репликации базы данных в реальном времени  
- Автоматического переключения на резервный инстанс при сбое  
- Health-check эндпоинта `/health` каждые 30 секунд

### 4.3. Требования к быстродействию, надёжности и масштабированию

| Параметр | Требование | Реализация |
|--------|-----------|-----------|
| SLA | ≥99% | Multi-AZ + auto-recovery |
| Общая задержка (e2e) | ≤5 сек | Ожидается: **≤2.5 сек** (1.8 сек — инференс на устройстве + 0.3–0.7 сек — API) |
| Пропускная способность | 100 RPS | Поддерживается Auto Scaling Group (от 2 до 10 инстансов) |
| Время восстановления | ≤15 мин | Обеспечено RDS failover + ALB |

**Стратегия масштабирования:**
- **Автоматическое масштабирование**: по CPU >70% (минимум 2, максимум 10 инстансов)  
- **Rate limiting**: 20 запросов/час/IP через API Gateway  
- **Кэширование**: планируется внедрение Redis для топ-20 объектов (пост-MVP)

### 4.4. Нагрузочное тестирование

Фактическое нагрузочное тестирование **не проводилось** из-за отсутствия production-стенда на этапе MVP. Однако выполнена **оценка производительности** на основе локального профилирования.

**План тестирования (будет реализован позже):**
- Инструмент: `locust`  
- Сценарий: 100 пользователей, 10 запросов `/history/{id}` каждый, за 60 сек  
- Цель: проверить стабильность при 100 RPS

**Ожидаемые результаты (на основе профилирования):**
- Средняя задержка: **120–180 мс**  
- Максимальная пропускная способность: **~120 RPS**  
- Доля ошибок (5xx): **<0.1%** при нагрузке ≤100 RPS

> График нагрузки будет добавлен в `/reports/load_test_simulation.png` после получения доступа к тестовому стенд.

### 4.5. Безопасность системы

- Все API-вызовы защищены **HTTPS (TLS 1.3)**  
- Аутентификация: **не требуется** для публичных запросов (но лимитирована по IP)  
- Шифрование данных:  
  - В покое: AES-256 (RDS, S3)  
  - На устройстве: AES-256 (для кэша, если будет реализован)  
- Регулярные **security-аудиты** и обновление зависимостей

### 4.6. Безопасность данных и соответствие GDPR

- Фотографии **не сохраняются и не передаются** на сервер  
- В логах хранятся только: `timestamp`, `object_id`, `device_type`, `request_duration`  
- Логи анонимизированы и хранятся **не более 30 дней**  
- Пользователь даёт **явное согласие** при первом запуске (GDPR-compliant popup)

### 4.7. Издержки

**Расчетные месячные затраты (AWS):**
...
| RDS (db.t4g.small) | $60 |
| API Gateway + ALB | $20 |
| CloudWatch + Data Transfer | $30 |
| Резерв (S3, мониторинг) | $20 |
| **Итого** | **~$250/мес** |

> Прогноз на 1000 активных пользователей/день. При росте нагрузки — автоматическое масштабирование с пропорциональным ростом затрат.
